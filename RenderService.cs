...
namespace RoleplayOverlay
{
  public sealed class RenderService
  {
      ...
      RenderSettings settings)
    {
      ...
      cmd.Append("-y ");
      bool hasComposedBase = !string.IsNullOrWhiteSpace(seq.ComposedClipPath) && File.Exists(seq.ComposedClipPath);
      if (hasComposedBase)
        cmd.Append($"-ss 0.20 -i \"{seq.ComposedClipPath}\" ");
      else if (slidePath != null)
      ...
      if (hasComposedBase)
        fc.Append($"[0:v]scale={vw}:{vh}:force_original_aspect_ratio=increase,crop={vw}:{vh}[bg];");
      else
    ...
    private static void BuildMediaFilterComplex(StringBuilder fc, Sequence seq, int mediaIdx, int canvasW, int canvasH)
    {
      ...
      var (ovx, ovy) = OverlayPosition(seq.MediaAnchor, Math.Max(0, (int)(canvasW * 0.03f)));
      if (blur > 0 && seq.MediaShadowAlpha > 0.01f)
      {
        ...
        fc.Append($"[bg][msh]overlay={ovx}+8:{ovy}+8:format=auto[bgs];");
        fc.Append($"[bgs][mfo]overlay={ovx}:{ovy}:format=auto[out]");
      }
      else
      {
        fc.Append($"[bg][media_framed]overlay={ovx}:{ovy}:format=auto[out]");
      }
    }
