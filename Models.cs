...
namespace RoleplayOverlay
{
  ...
  public sealed class Sequence : INotifyPropertyChanged
  {
    ...
    private string? _composedClipPath;
    private double  _camX = 0.80;
    private double  _camY = 0.84;
    private double  _camDiam = 0.34;
    private bool    _camRelift = true;
    ...
    private string  _mediaAnchor = "center";
    [JsonProperty("id")]
    ...
    public string? CamClipPath
    {
      get => _camClipPath;
      set { if (SetField(ref _camClipPath, value)) { OnPropertyChanged(nameof(ClipBadge)); OnPropertyChanged(nameof(ClipTooltip)); } }
    }
    [JsonProperty("composedClipPath", NullValueHandling = NullValueHandling.Ignore)]
    public string? ComposedClipPath
    {
      get => _composedClipPath;
      set { if (SetField(ref _composedClipPath, value)) { OnPropertyChanged(nameof(ClipBadge)); OnPropertyChanged(nameof(ClipTooltip)); } }
    }
    [JsonProperty("camX", DefaultValueHandling = DefaultValueHandling.Ignore)]
    public double CamX { get => _camX; set => SetField(ref _camX, value); }
    [JsonProperty("camY", DefaultValueHandling = DefaultValueHandling.Ignore)]
    public double CamY { get => _camY; set => SetField(ref _camY, value); }
    [JsonProperty("camDiam", DefaultValueHandling = DefaultValueHandling.Ignore)]
    public double CamDiam { get => _camDiam; set => SetField(ref _camDiam, value); }
    [JsonProperty("camRelift", DefaultValueHandling = DefaultValueHandling.Ignore)]
    public bool CamRelift { get => _camRelift; set => SetField(ref _camRelift, value); }
    [JsonIgnore]
    public string ClipBadge =>
      !string.IsNullOrWhiteSpace(_composedClipPath) ? "🎬"
      : !string.IsNullOrWhiteSpace(_camClipPath) ? "📷"
      : "";
    [JsonIgnore]
    public string? ClipTooltip =>
      !string.IsNullOrWhiteSpace(_composedClipPath) ? "Clip composé : " + System.IO.Path.GetFileName(_composedClipPath)
      : !string.IsNullOrWhiteSpace(_camClipPath) ? "Clip cam : " + System.IO.Path.GetFileName(_camClipPath)
      : null;
    [JsonProperty("text")]
    ...
    public string MediaAnchor { get => _mediaAnchor; set => SetField(ref _mediaAnchor, value); }
    ...
    public Sequence Clone()
    {
      ...
      {
        _id = _id, _mode = _mode, _mp3 = _mp3, _camClipPath = _camClipPath,
        _composedClipPath = _composedClipPath,
        _camX = _camX, _camY = _camY, _camDiam = _camDiam, _camRelift = _camRelift, _text = _text,
