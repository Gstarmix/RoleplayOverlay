...
internal sealed class VideoRecorder
{
    ...
    private AudioCapture? _prewarmedAudio;
    private bool _showCursor = true;
    private bool _maskWm;
    ...
    public void Start(Rectangle zone, string path, int fps = 30, AudioOptions? audio = null,
        AudioCapture? prewarmedAudio = null, bool showCursor = true, bool maskWinWatermark = false)
    {
        if (_running) return;
        int w = zone.Width & ~1;
        int h = zone.Height & ~1;
        ...
        _prewarmedAudio = prewarmedAudio;
        _showCursor = showCursor;
        _maskWm = maskWinWatermark;
        _error = null;
        _running = true;
    ...
    private void Loop()
    {
        ...
        try
        {
            ...
            if (audioIndex >= 0)
            {
                audio = _prewarmedAudio ?? new AudioCapture();
                int ai = audioIndex;
            ...
            while (_running)
            {
                ...
                if (_showCursor) DrawCursorOverlay(g, _zone);
                if (_maskWm) MaskWinWatermark(g, bmp, _zone);
    ...
    private static void DrawCursorOverlay(Graphics g, Rectangle zone)
    {
        var ci = new Native.CURSORINFO { cbSize = Marshal.SizeOf<Native.CURSORINFO>() };
        if (!Native.GetCursorInfo(ref ci)) return;
        if ((ci.flags & Native.CURSOR_SHOWING) == 0 || ci.hCursor == IntPtr.Zero) return;
        int x = ci.ptScreenPos.X - zone.X;
        int y = ci.ptScreenPos.Y - zone.Y;
        if (x < -64 || y < -64 || x > zone.Width + 64 || y > zone.Height + 64) return;
        if (!Native.GetIconInfo(ci.hCursor, out var info)) return;
        try
        {
            IntPtr hdc = g.GetHdc();
            try { Native.DrawIcon(hdc, x - info.xHotspot, y - info.yHotspot, ci.hCursor); }
    ...
    private static void MaskWinWatermark(Graphics g, Bitmap bmp, Rectangle zone)
    {
        var scr = System.Windows.Forms.Screen.PrimaryScreen?.Bounds ?? Rectangle.Empty;
        if (scr.IsEmpty) return;
        const int wmW = 252, wmH = 66;
        var wm = new Rectangle(scr.Right - wmW, scr.Bottom - wmH, wmW, wmH);
        wm.Intersect(zone);
        if (wm.Width < 4 || wm.Height < 4) return;
        var frameRect = new Rectangle(0, 0, bmp.Width, bmp.Height);
        var box = new Rectangle(wm.X - zone.X, wm.Y - zone.Y, wm.Width, wm.Height);
        box.Intersect(frameRect);
        if (box.Width < 4 || box.Height < 4) return;
        int sy = box.Y - box.Height;
        if (sy < 0) { sy = box.Y + box.Height; if (sy + box.Height > bmp.Height) return; }
        var src = new Rectangle(box.X, sy, box.Width, box.Height);
        if (!frameRect.Contains(src)) return;
        using var patch = bmp.Clone(src, bmp.PixelFormat);
        g.DrawImageUnscaled(patch, box.X, box.Y);
    }
