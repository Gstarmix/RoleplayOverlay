...
internal static class Teleprompter
{
    public static string ScriptsDir
    {
        get
        {
            ...
            {
                string cand = Path.Combine(dir.FullName, "scripts");
    ...
    public static List<string> SplitSegments(string raw)
    {
...
internal sealed class TeleprompterForm : Form
{
    ...
    private void Opac(double d) { Opacity = Math.Clamp(Opacity + d, 0.25, 1.0); SettingsChanged?.Invoke(); }
    ...
    public void RestoreWindowBounds(Rectangle r)
    {
        if (!IsHandleCreated) return;
        Native.SetWindowPos(Handle, Native.HWND_TOPMOST, r.X, r.Y, r.Width, r.Height,
