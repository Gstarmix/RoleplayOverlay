...
internal interface IFrameSource
{
    ...
    void DrawTo(Graphics g, Rectangle dest, ImageAttributes? attrs = null);
    void DrawCenteredSquare(Graphics g, Rectangle dest, double panX = 0, double panY = 0, ImageAttributes? attrs = null);
    void DrawCover(Graphics g, Rectangle dest, double panX = 0, double panY = 0, ImageAttributes? attrs = null);
}
internal sealed class WebcamReader : IFrameSource
{
    ...
    public void DrawCenteredSquare(Graphics g, Rectangle dest, double panX = 0, double panY = 0, ImageAttributes? attrs = null)
    {
        lock (_lock)
        {
            ...
            var (sx, sy) = PannedOrigin(_frame.Width, _frame.Height, side, side, panX, panY);
    ...
    public void DrawCover(Graphics g, Rectangle dest, double panX = 0, double panY = 0, ImageAttributes? attrs = null)
    {
        lock (_lock)
        {
            ...
            var (sx, sy) = PannedOrigin(_frame.Width, _frame.Height, sw, sh, panX, panY);
    ...
    internal static (int X, int Y) PannedOrigin(int frameW, int frameH, int sw, int sh, double panX, double panY)
    {
        int slackX = frameW - sw, slackY = frameH - sh;
        int sx = (int)Math.Round(Math.Clamp((slackX / 2.0) * (1 + panX), 0, slackX));
        int sy = (int)Math.Round(Math.Clamp((slackY / 2.0) * (1 + panY), 0, slackY));
        return (sx, sy);
    }
...
internal sealed class CamForm : Form
{
    ...
    private readonly IFrameSource _reader;
    ...
    private Color _borderColor = Color.FromArgb(230, 255, 255, 255);
    private int   _borderPx    = 3;
    private double? _aspectOverride;
    private const int Grip = 22;
    private const int Radius = 28;
    private double _panX, _panY;
    private Point? _rightDragScreenStart;
    private double _panXAtRightDragStart, _panYAtRightDragStart;
    private bool _rightDragMoved;
    public event EventHandler? CloseRequested;
    public event EventHandler? GeometryChanged;
    public event EventHandler? PanChanged;
    public Rectangle? AllowedBounds { get; set; }
    public bool Mirror
    {
        get => _mirror;
        set { if (_mirror != value) { _mirror = value; Invalidate(); } }
    }
    public CamForm(IFrameSource reader, Shape shape = Shape.Rectangle, bool mirror = false,
    ...
    private double TargetAspect => _shape == Shape.Circle ? 1.0 : (_aspectOverride ?? _camAspect);
    public void SetBorder(Color color, int px) { _borderColor = color; _borderPx = Math.Max(0, px); Invalidate(); }
    public double PanX => _panX;
    public double PanY => _panY;
    public void SetPan(double panX, double panY)
    {
        _panX = Math.Clamp(panX, -1, 1);
        _panY = Math.Clamp(panY, -1, 1);
        Invalidate();
    }
    public void SetAspectOverride(double? aspect, Rectangle zone)
    {
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        {
            if (_shape == Shape.Circle) _reader.DrawCenteredSquare(g, cr, _panX, _panY);
            else _reader.DrawCover(g, cr, _panX, _panY);
        }
        else if (_filterParams.Smoothing > 0 || _filterParams.Sharpness > 0f)
        {
            ...
            using (var g2 = Graphics.FromImage(tmp))
            {
                ...
                if (_shape == Shape.Circle) _reader.DrawCenteredSquare(g2, cr, _panX, _panY);
                else _reader.DrawCover(g2, cr, _panX, _panY);
            }
        ...
        {
            using var attrs = CameraFilter.BuildAttributes(_filterParams);
            if (_shape == Shape.Circle) _reader.DrawCenteredSquare(g, cr, _panX, _panY, attrs);
            else _reader.DrawCover(g, cr, _panX, _panY, attrs);
        }
        g.Restore(state);
        ...
        if (_borderPx > 0)
        {
            ...
            using var pen = new Pen(_borderColor, _borderPx);
            switch (_shape)
            {
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        ...
        _rightDragScreenStart = PointToScreen(e.Location);
        _panXAtRightDragStart = _panX;
        _panYAtRightDragStart = _panY;
    ...
    {
        base.OnMouseMove(e);
        if (_rightDragScreenStart is not Point start) return;
        Point cur = PointToScreen(e.Location);
        int dx = cur.X - start.X, dy = cur.Y - start.Y;
        if (!_rightDragMoved && (Math.Abs(dx) > 3 || Math.Abs(dy) > 3)) _rightDragMoved = true;
        if (!_rightDragMoved) return;
        _panX = Math.Clamp(_panXAtRightDragStart - dx * 2.0 / Math.Max(1, Width), -1, 1);
        _panY = Math.Clamp(_panYAtRightDragStart - dy * 2.0 / Math.Max(1, Height), -1, 1);
        Invalidate();
    ...
    protected override void OnMouseUp(MouseEventArgs e)
    {
        ...
        if (e.Button != MouseButtons.Right) return;
        Capture = false;
        bool wasDrag = _rightDragMoved;
        _rightDragScreenStart = null;
        _rightDragMoved = false;
        if (!wasDrag) CloseRequested?.Invoke(this, EventArgs.Empty);
    }
    protected override void OnKeyDown(KeyEventArgs e)
    {
        base.OnKeyDown(e);
        if (e.KeyCode == Keys.Escape) { CloseRequested?.Invoke(this, EventArgs.Empty); return; }
        const double step = 0.12;
        switch (e.KeyCode)
        {
            case Keys.Left:  _panX = Math.Clamp(_panX - step, -1, 1); break;
            case Keys.Right: _panX = Math.Clamp(_panX + step, -1, 1); break;
            case Keys.Up:    _panY = Math.Clamp(_panY - step, -1, 1); break;
            case Keys.Down:  _panY = Math.Clamp(_panY + step, -1, 1); break;
            default: return;
        }
        Invalidate();
        PanChanged?.Invoke(this, EventArgs.Empty);
    ...
    protected override void WndProc(ref Message m)
    {
        ...
        const int HTCLIENT = 1, HTCAPTION = 2, HTBOTTOMRIGHT = 17;
        ...
        if (m.Msg == WM_NCHITTEST)
        {
            ...
            if (pt.X >= Width - Grip && pt.Y >= Height - Grip) { m.Result = new IntPtr(HTBOTTOMRIGHT); return; }
            bool rightDown = (Control.MouseButtons & MouseButtons.Right) != 0;
            m.Result = new IntPtr(rightDown ? HTCLIENT : HTCAPTION);
            return;
        }
        if (m.Msg == WM_MOVING && AllowedBounds.HasValue)
        {
            var rc = Marshal.PtrToStructure<Native.RECT>(m.LParam);
            var b = AllowedBounds.Value;
            int w = rc.Right - rc.Left, h = rc.Bottom - rc.Top;
            int left = Math.Clamp(rc.Left, b.X, Math.Max(b.X, b.Right - w));
            int top  = Math.Clamp(rc.Top,  b.Y, Math.Max(b.Y, b.Bottom - h));
            rc.Left = left; rc.Right = left + w;
            rc.Top  = top;  rc.Bottom = top + h;
        ...
        if (m.Msg == WM_SIZING)
        {
            ...
            int h = (int)Math.Round(w / TargetAspect);
            if (AllowedBounds.HasValue)
            {
                var b = AllowedBounds.Value;
                int maxW = Math.Max(MinimumSize.Width,  b.Right  - rc.Left);
                int maxH = Math.Max(MinimumSize.Height, b.Bottom - rc.Top);
                if (w > maxW) w = maxW;
                h = (int)Math.Round(w / TargetAspect);
                if (h > maxH) { h = maxH; w = (int)Math.Round(h * TargetAspect); }
                rc.Right = rc.Left + w;
            }
            rc.Bottom = rc.Top + h;
            Marshal.StructureToPtr(rc, m.LParam, false);
...
internal sealed class CountdownForm : Form
{
    ...
    public CountdownForm(Rectangle zone, int seconds, Action onElapsed)
    {
        ...
        DoubleBuffered = true;
        Text = "RoleplayOverlay_Countdown";
        _timer.Tick += OnTick;
    }
...
internal sealed class BackdropForm : Form
{
    ...
    public void SetSlide(Bitmap? slide)
    {
        var old = _slide;
        _slide = slide;
        old?.Dispose();
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        var g = e.Graphics;
        var cr = ClientRectangle;
        if (_slide == null || cr.Width <= 0 || cr.Height <= 0)
        {
            g.Clear(Color.Black);
            return;
        }
        double target = (double)cr.Width / cr.Height;
        double src = (double)_slide.Width / _slide.Height;
        int sw, sh;
        if (src > target) { sh = _slide.Height; sw = (int)Math.Round(sh * target); }
        else { sw = _slide.Width; sh = (int)Math.Round(sw / target); }
        int sx = (_slide.Width - sw) / 2;
        int sy = (_slide.Height - sh) / 2;
        g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBilinear;
        g.DrawImage(_slide, cr, new Rectangle(sx, sy, sw, sh), GraphicsUnit.Pixel);
    }
    protected override void OnFormClosed(FormClosedEventArgs e)
    {
        _slide?.Dispose();
